...
import { useSettingsStore, FontFamily, WordSpacing, LetterSpacing, FontSize } from '@/state/stores/settingsStore';
...
function FontSettingsPageContent() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) {
      console.log('🔒 Font Settings - 미인증 상태, 로그인 페이지로 이동');
      router.push('/login?redirect=%2Fsettings%2Ffont');
    }
...
export default function FontSettingsPage() {
  return <FontSettingsPageContent />;
}