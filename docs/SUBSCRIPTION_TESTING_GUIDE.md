...
| 재결제 테스트 | Cancel 후 바로 가능 | 3단계 초기화 필요 |
...
WHERE user_id = '...' AND platform = 'apple';
```
...
기존 구독 이력이 남아있으면 Apple이 "이미 구독 중"으로 판단하므로, 세 곳 모두 초기화해야 한다.
...
```sql
-- 상태 확인
SELECT id, user_id, status, platform, updated_at
FROM subscriptions
WHERE user_id = '...';

-- 수동 만료 (비구독 테스트)
UPDATE subscriptions SET status = 'expired', updated_at = now()
WHERE user_id = '...' AND platform = 'paddle';  -- 또는 'apple'
...
-- 레코드 삭제 (완전 초기화)
DELETE FROM subscriptions WHERE user_id = '...' AND platform = 'apple';
```
...
| 재결제 시 "이미 구독 중" (Apple) | Apple/RevenueCat에 이전 이력 잔존 | 3단계 초기화 수행 |
