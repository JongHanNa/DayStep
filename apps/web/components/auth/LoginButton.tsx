'use client';

import React from 'react';
import { useAuth } from '@/app/context/AuthContext';
import { Button } from '@/components/ui/button';

interface LoginButtonProps {
  provider: 'google' | 'test-account';
  className?: string;
}

/**
 * 소셜 로그인 버튼 컴포넌트
 */
export function LoginButton({ provider, className }: LoginButtonProps) {
  const { signInWithGoogle, signInWithTestAccount } = useAuth();

  const handleLogin = async () => {
    try {
      if (provider === 'google') {
        await signInWithGoogle();
      } else if (provider === 'test-account') {
        await signInWithTestAccount();
      }
    } catch (error) {
      console.error(`${provider} 로그인 오류:`, error);
    }
  };

  const getButtonText = () => {
    switch (provider) {
      case 'google':
        return 'Google로 로그인';
      case 'test-account':
        return '테스트 계정으로 로그인';
      default:
        return '로그인';
    }
  };

  const getButtonIcon = () => {
    switch (provider) {
      case 'google':
        return (
          <svg className="w-6 h-6 mr-3" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
        );
      case 'test-account':
        return (
          <svg className="w-6 h-6 mr-3" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
        );
      default:
        return null;
    }
  };

  const getButtonStyles = () => {
    switch (provider) {
      case 'google':
        return 'bg-white hover:bg-gray-50 text-gray-800 border border-gray-200 shadow-md hover:shadow-lg dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600 dark:hover:bg-gray-600';
      case 'test-account':
        return 'bg-green-600 hover:bg-green-700 text-white border-0 shadow-md hover:shadow-lg';
      default:
        return 'bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg';
    }
  };

  return (
    <Button
      onClick={handleLogin}
      className={`w-full flex items-center justify-center py-6 px-8 text-lg font-bold rounded-2xl transition-all duration-300 transform hover:scale-[1.02] ${getButtonStyles()} ${className}`}
      variant="outline"
    >
      {getButtonIcon()}
      {getButtonText()}
    </Button>
  );
}