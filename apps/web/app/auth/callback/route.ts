...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.warn('[OAuth 콜백] URL 파라미터:', {
      fullUrl: request.url,
      hasCode: !!code,
      hasNext: !!next,
    ...
    console.warn('[OAuth 콜백] exchangeCodeForSession 호출 시작');

    // OAuth 코드를 세션으로 교환 (PKCE 자동 처리)
    ...
    console.warn('[OAuth 콜백] 리다이렉트 대상:', redirectTo);

    // 302 리다이렉트로 브라우저 히스토리에서 OAuth 코드 제거
