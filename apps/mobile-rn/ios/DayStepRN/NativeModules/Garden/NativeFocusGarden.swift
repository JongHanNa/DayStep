/**
 * NativeFocusGarden — SwiftUI 집중 정원 4-뷰 컴포넌트
 * 일: 5×5 아이소메트릭 다이아몬드 정원
 * 주/월/년: 수면정원/청소정원과 유사한 패턴
 *
 * 청소 정원과 차이점:
 *   - tab 개념 제거 (집중은 단일 카테고리)
 *   - crownColor: primaryColor 기반 단일 팔레트 (HSL/RGB lightness 변주)
...
struct FocusTreeInfo: Codable {
  let sessionId: String
  let durationSeconds: Int
  let outcome: String  // "completed" | "abandoned"
}

struct FocusGardenDayInfo: Codable {
  let date: String  // yyyy-MM-dd
  let trees: [FocusTreeInfo]
}

struct FocusGardenPayload: Codable {
  let days: [FocusGardenDayInfo]
}

// MARK: - Observable State

class FocusGardenState: ObservableObject {
  @Published var viewMode: String = "day"
  @Published var selectedDate: String = ""
  @Published var primaryColor: String = "#3B82F6"
  @Published var parsedDays: [String: [FocusTreeInfo]] = [:]
  ...
  func aggregatedTrees(for dates: [String]) -> [FocusTreeInfo] {
...
// MARK: - Focus Tree Canvas View

struct FocusTreeCanvasView: View {
  let durationSeconds: Int
  let outcome: String
  ...
  /// primaryColor를 white와 섞어 레벨별 크라운 색상 생성
  /// 레벨 0 (씨앗/옅음) → 원색과 55% white mix
  /// 레벨 3 (원색) → 혼합 없음
  private var crownColor: Color {
    if isAbandoned { return Color(hex: "#9CA3AF") }
    let base = Color(hex: primaryColorHex)
    let mix: Double = {
      switch growthLevel {
      case 3: return 0.0
      case 2: return 0.1
    ...
    return base.mixed(with: .white, amount: mix)
  }
...
struct IsometricFocusGardenView: View {
  @ObservedObject var state: FocusGardenState
  let todayStr: String
  ...
  private var trees: [FocusTreeInfo] {
          ...
          ForEach(0..<gridSize, id: \.self) { col in
            ...
            if let t = tree {
              FocusTreeCanvasView(
                ...
                outcome: t.outcome,
                primaryColorHex: state.primaryColor,
                size: tileWidth * 0.65
              )
  ...
  private var completedText: some View {
    ...
    return Group {
      if completedCount > 0 {
        ...
        if prefix.isEmpty {
          Text("\(completedCount)개 집중 완료")
            .font(.system(size: 15, weight: .semibold))
            .foregroundColor(Color(hex: state.primaryColor))
        } else {
          Text("\(prefix) \(completedCount)개 집중 완료")
            .font(.system(size: 15, weight: .semibold))
            .foregroundColor(Color(hex: state.primaryColor))
        }
      } else {
  ...
  private func tilePosition(row: Int, col: Int) -> CGPoint {
    ...
    let topY = CGFloat(gridSize) * tileHeight / 2 + 30
...
struct FocusGardenContent: View {
  @ObservedObject var state: FocusGardenState
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      // IsometricFocusGardenView 하나가 4개 뷰모드를 모두 처리 (청소 정원과 동일 패턴)
      IsometricFocusGardenView(
        state: state,
        todayStr: todayStr,
        calendar: calendar,
  ...
  private var legendView: some View {
    HStack(spacing: 12) {
      legendItem(seconds: 600, outcome: "completed", label: "10m+")
      legendItem(seconds: 420, outcome: "completed", label: "5-10m")
      legendItem(seconds: 180, outcome: "completed", label: "2-5m")
      legendItem(seconds: 300, outcome: "abandoned", label: "중단")
    }
  }

  private func legendItem(seconds: Int, outcome: String, label: String) -> some View {
    HStack(spacing: 4) {
      FocusTreeCanvasView(durationSeconds: seconds, outcome: outcome, primaryColorHex: state.primaryColor, size: 14)
      Text(label)
        .font(.system(size: 11))
        .foregroundColor(Color(hex: "#9CA3AF"))
    }
      ...
      VStack(spacing: 4) {
        ...
        VStack(spacing: 2) {
          ForEach(Array(trees.prefix(3).enumerated()), id: \.offset) { _, tree in
            FocusTreeCanvasView(
              durationSeconds: tree.durationSeconds,
...
struct FocusYearView: View {
  ...
  private func heatmapColor(status: String) -> Color {
    switch status {
    case "healthy": return Color(hex: state.primaryColor)
    case "wilted": return Color(hex: "#D1D5DB")
    case "today": return Color(hex: state.primaryColor).opacity(0.4)
...
extension Color {
  /// self와 다른 색을 amount (0~1) 비율로 혼합. 0이면 self, 1이면 other.
  func mixed(with other: Color, amount: Double) -> Color {
    let selfComponents = UIColor(self).rgba
    let otherComponents = UIColor(other).rgba
    let r = selfComponents.r + (otherComponents.r - selfComponents.r) * amount
    let g = selfComponents.g + (otherComponents.g - selfComponents.g) * amount
    let b = selfComponents.b + (otherComponents.b - selfComponents.b) * amount
    let a = selfComponents.a + (otherComponents.a - selfComponents.a) * amount
...
class NativeFocusGardenUIView: UIView {
  ...
  private let gardenState = FocusGardenState()
  ...
  @objc func setGardenData(_ value: NSString) {
    ...
    do {
      let payload = try JSONDecoder().decode(FocusGardenPayload.self, from: data)
      var map: [String: [FocusTreeInfo]] = [:]
      for day in payload.days {
        map[day.date] = day.trees
      }
      gardenState.parsedDays = map
    } catch {
      print("[NativeFocusGarden] gardenData decode error: \(error)")
    }
    setupOnce()
  ...
  private func setupOnce() {
    ...
    let swiftUIView = FocusGardenContent(
      state: gardenState,
...
@objc(NativeFocusGardenManager)
class NativeFocusGardenManager: RCTViewManager {
  override func view() -> UIView! {
    return NativeFocusGardenUIView()
  }
