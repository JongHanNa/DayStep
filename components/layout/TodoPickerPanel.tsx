...
export default function TodoPickerPanel() {
  ...
  const handleTaskSelect = (task: PresetTask) => {
    console.log(`작업 선택: ${task.title} (${task.duration}분)`);
    // TODO: 실제 타임라인 추가 로직 구현
    setIsOpen(false);
  };
  ...
  return (
    <>
      {/* react-modal-sheet를 사용한 작업 선택 패널 */}
      <Sheet
        isOpen={isOpen}
      ...
      <div className="fixed bottom-16 left-1/2 transform -translate-x-1/2 z-50">
        <button
          ...
          className="bg-brand hover:bg-brand-hover text-white p-2 rounded-2xl shadow-sm transition-all duration-200 hover:scale-105"
          aria-label="할일 선택 패널 열기"
        >
