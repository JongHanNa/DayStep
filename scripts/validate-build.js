...
function validateBuildTarget(target) {
  ...
  if (buildTarget === 'mobile') {
    // 모바일 빌드 요구사항 검증
    if (!fs.existsSync('mobile/capacitor.config.ts')) {
      logError('Capacitor 설정 파일이 없습니다: mobile/capacitor.config.ts');
      return false;
    }
    
    if (!fs.existsSync('mobile/ios') && !fs.existsSync('mobile/android')) {
